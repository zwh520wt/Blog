...
import com.example.springboot.dao.mapper.ArticleBodyMapper;
import com.example.springboot.dao.mapper.ArticleMapper;
import com.example.springboot.dao.mapper.ArticleTagMapper;
import com.example.springboot.dao.pojo.Article;
import com.example.springboot.dao.pojo.ArticleBody;
import com.example.springboot.dao.pojo.ArticleTag;
...
//    }
    ...
    public Result hotArticle(int limit) {
        ...
        //上述语句的意思 select id,title from article order by view_counts desc limit 5
    ...
    public Result newArticles(int limit) {
        ...
        queryWrapper.select(Article::getId, Article::getTitle);
        queryWrapper.last("limit " + limit);
        //上述语句的意思 select id,title from article order by create_date desc limit 5
        List<Article> articles = articleMapper.selectList(queryWrapper);
        return Result.success(copyList(articles, false, false));
    }
    ...
    public Result findArticleById(Long articleId) {
        ...
        //查看完文章了，新增阅读量，有没有问题
        //查看文章之后，本应该直接返回数据了，这时候做一个更新操作，更新时加写锁，阻塞其他的读操作，性能就会比较低
        //更新操作   增加了接口的耗时。 如果一旦更新出问题，不能影响 查看文章的操作
        // 使用线程池 来解决 可以把更新操作放入到线程池中进行执行，和主线程就不相关了
    ...
    public Result publish(ArticleParam articleParam) {
        ...
        if (tags != null) {
            for (TagVo tag : tags) {
                ...
                articleTag.setTagId(Long.parseLong(String.valueOf(tag.getId())));
